# 📦 Полная инструкция по установке

## Структура проекта

Проект разделен на **2 независимых компонента**:

```
tg-mtproxy/
├── control-panel/     # Telegram бот для управления
│   ├── src/
│   ├── scripts/
│   │   ├── install.sh
│   │   └── uninstall.sh
│   └── README.md
│
└── node-agent/        # Агент на каждом прокси-сервере
    ├── src/
    ├── scripts/
    │   ├── install.sh
    │   └── uninstall.sh
    └── README.md
```

---

## 🎯 Сценарий 1: Все на одном сервере

Для тестирования или небольшого количества пользователей.

### Шаг 1: Подготовка сервера

```bash
# Обновите систему
sudo apt update && sudo apt upgrade -y

# Клонируйте репозиторий
git clone https://github.com/goodboy34-tech/eeee.git
cd eeee
```

### Шаг 2: Установка Node Agent

```bash
cd node-agent
sudo bash scripts/install.sh
```

**Что нужно сделать ДО установки:**
```bash
# 1. Скопировать .env.example в .env
cp .env.example .env

# 2. Сгенерировать ключи
openssl rand -hex 32  # API Key - сохраните!
openssl rand -hex 16  # MTProxy Secret

# 3. Отредактировать .env
nano .env
# Заполните: NODE_NAME, DOMAIN, API_KEY, SECRET, WORKERS

# 4. Только теперь запускать установку
sudo bash scripts/install.sh
```

**Подробнее:** [ENV-SETUP.md](./ENV-SETUP.md)

### Шаг 3: Установка Control Panel

```bash
cd ../control-panel
sudo bash scripts/install.sh
```

**Что нужно сделать ДО установки:**
```bash
# 1. Скопировать .env.example в .env
cp .env.example .env

# 2. Получить Bot Token от @BotFather
# 3. Получить свой Telegram ID от @userinfobot

# 4. Отредактировать .env
nano .env
# BOT_TOKEN=ваш_токен
# ADMIN_IDS=ваш_id

# 5. Только теперь запускать установку
sudo bash scripts/install.sh
```

**Подробнее:** [ENV-SETUP.md](./ENV-SETUP.md)

### Шаг 4: Подключение ноды

Откройте вашего бота в Telegram и отправьте:

```
/add_node Node1 your-domain.com https://YOUR_IP:3001 YOUR_API_KEY
```

### Шаг 5: Создание прокси

```
# MTProto с Fake-TLS
/add_secret 1 dd

# SOCKS5 с авторизацией
/add_socks5 1

# Получить ссылки
/links 1
```

✅ **Готово!** Вы можете использовать полученные ссылки в Telegram.

---

## 🌐 Сценарий 2: Control Panel и ноды на разных серверах

Рекомендуется для production.

### Архитектура

```
Control Panel Server    →   Node 1 Server
(любой)                     (публичный IP)
  
                        →   Node 2 Server
                            (публичный IP)
                            
                        →   Node N Server
                            (публичный IP)
```

### Установка Control Panel

**На сервере управления:**

```bash
# 1. Клонировать
git clone https://github.com/goodboy34-tech/eeee.git
cd eeee/control-panel

# 2. Настроить .env
cp .env.example .env
nano .env
# Укажите BOT_TOKEN и ADMIN_IDS (см. ENV-SETUP.md)

# 3. Установить
sudo bash scripts/install.sh
```

**Как получить токены - смотрите [ENV-SETUP.md](./ENV-SETUP.md)**

### Установка Node Agent на каждом сервере

**На каждом прокси-сервере повторить:**

```bash
# 1. Клонировать
git clone https://github.com/goodboy34-tech/eeee.git
cd eeee/node-agent

# 2. Сгенерировать ключи
echo "API_KEY: $(openssl rand -hex 32)"
echo "SECRET: $(openssl rand -hex 16)"
# ⚠️ СОХРАНИТЕ API_KEY!

# 3. Настроить .env
cp .env.example .env
nano .env
# Заполните: NODE_NAME, DOMAIN, API_KEY, SECRET, WORKERS

# 4. Установить
sudo bash scripts/install.sh
```

**Подробная инструкция - смотрите [ENV-SETUP.md](./ENV-SETUP.md)**

### Подключение всех нод

В Telegram боте:

```
/add_node Node1 proxy1.example.com https://IP1:3001 API_KEY1
/add_node Node2 proxy2.example.com https://IP2:3001 API_KEY2
/add_node Node3 proxy3.example.com https://IP3:3001 API_KEY3
```

### Создание прокси на всех нодах

```
# Для каждой ноды:
/add_secret <node_id> dd
/add_socks5 <node_id>
```

### Создание подписки с несколькими нодами

```
/create_subscription

# Выберите ноду
# Укажите количество MTProto: 3
# Укажите количество SOCKS5: 3
```

Пользователь получит подписку с прокси со всех нод!

---

## 🔐 Важные моменты

### API Key

**Где хранится:** в файле `.env` на каждой ноде

**Как посмотреть:**
```bash
cat /path/to/node-agent/.env | grep API_KEY
```

**Как изменить:**
```bash
# На ноде
cd /path/to/node-agent
nano .env
# Измените API_KEY=...
docker-compose restart

# В Control Panel
/node <node_id>
# Обновите API ключ через интерфейс
```

### Firewall

Автоматически открываются порты:
- **443** - MTProxy
- **1080** - SOCKS5
- **3001** - Node API

**Дополнительная защита API:**
```bash
# Закрыть API для всех
ufw delete allow 3001/tcp

# Открыть только для Control Panel
ufw allow from CONTROL_PANEL_IP to any port 3001 proto tcp
ufw reload
```

### SSL сертификаты

По умолчанию используются самоподписанные сертификаты.

**Для production с Let's Encrypt:**
```bash
# На каждой ноде
sudo apt install certbot
sudo certbot certonly --standalone -d api.yourdomain.com

# Обновите docker-compose.yml:
volumes:
  - /etc/letsencrypt/live/api.yourdomain.com:/certs:ro
```

---

## 📊 Проверка работоспособности

### Control Panel

```bash
cd control-panel

# Статус
docker-compose ps

# Логи
docker-compose logs -f

# Должно быть:
# ✅ mtproxy-control-panel Up
# 🤖 MTProxy Management Bot запущен!
```

### Node Agent

```bash
cd node-agent

# Статус
docker-compose ps

# Должно быть 3 контейнера:
# ✅ node-agent     Up
# ✅ mtproxy        Up
# ✅ socks5         Up

# Тест API
curl -k https://localhost:3001/health
# {"status":"ok",...}
```

### В Telegram

```
/nodes          # Должны увидеть все ноды со статусом ✅
/health 1       # Проверка конкретной ноды
/stats 1        # Статистика
```

---

## 🔄 Обновление системы

### Control Panel

```bash
cd control-panel
git pull
docker-compose down
docker-compose up -d --build
```

### Node Agent

```bash
cd node-agent
git pull
docker-compose down
docker-compose up -d --build
```

---

## 🗑️ Удаление

### Удалить Control Panel

```bash
cd control-panel
sudo bash scripts/uninstall.sh

# Выберите:
# Сохранить базу данных? (y/n)
# Продолжить удаление? (y/n)
```

### Удалить Node Agent

```bash
cd node-agent
sudo bash scripts/uninstall.sh

# Выберите:
# Сохранить конфигурацию? (y/n)
# Продолжить удаление? (y/n)
```

Скрипты удаления:
- ✅ Останавливают контейнеры
- ✅ Удаляют образы Docker
- ✅ Очищают firewall
- ✅ Удаляют файлы проекта
- ✅ Опционально сохраняют данные

---

## ❓ Частые проблемы

### Бот не отвечает

```bash
# Проверить логи
cd control-panel
docker-compose logs -f

# Проверить ADMIN_IDS
cat .env | grep ADMIN_IDS

# Перезапустить
docker-compose restart
```

### Нода недоступна

```bash
# Проверить статус на ноде
cd node-agent
docker-compose ps

# Проверить логи
docker-compose logs -f

# Проверить firewall
ufw status

# Тест API
curl -k https://localhost:3001/health
```

### MTProxy не работает

```bash
# Проверить логи MTProxy
docker-compose logs -f mtproxy

# Проверить порт
netstat -tupln | grep 443

# Пересоздать контейнер
docker-compose up -d --force-recreate mtproxy
```

### SOCKS5 не работает

```bash
# Проверить логи SOCKS5
docker-compose logs -f socks5

# Тест подключения
curl -x socks5://username:password@localhost:1080 https://ifconfig.me

# Пересоздать
docker-compose up -d --force-recreate socks5
```

---

## 📚 Дополнительные ресурсы

- [Control Panel README](./control-panel/README.md) - полная документация бота
- [Node Agent README](./node-agent/README.md) - полная документация агента
- [Основной README](./README.md) - обзор системы

---

## 🎯 Следующие шаги

После установки:

1. ✅ Создайте прокси на всех нодах
2. ✅ Настройте автоматический мониторинг (уже работает)
3. ✅ Создайте подписки для пользователей
4. ✅ Настройте резервное копирование базы данных
5. ✅ Мониторьте нагрузку и добавляйте ноды при необходимости

**Готово к работе!** 🎉
